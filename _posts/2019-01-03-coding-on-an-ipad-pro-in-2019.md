...
**Update**: if you code on an iPad, check out my detailed look at [ways to synchronize the iOS clipboard with a remote machine over SSH](https://andrewbrookins.com/technology/synchronizing-the-ios-clipboard-with-a-remote-server-using-command-line-tools/). I also explore [how to debug web apps from iPadOS](/technology/coding-on-ipados-debugging-web-applications-with-inspect/) using the Inspect app in a recent post.
