...
- iOS automatically backs itself up to the internet via iCloud
...
The most important part of the experiment was that my development environment -- my code and dependent programs -- would exist outside of the device that I use. Local caching should exist for rare offline use. The environment should provide roughly the same level of intelligence that I expect from PC and Mac IDEs.
...
It took some time to set up my disk, a compute node, get some projects set up, and script the process. In the meantime, I opened up the SSH port on my router and macOS firewall on my Macbook Pro, so I could try out an SSH-based working environment immediately.
...
- Blink, a terminal that can remap the Capslock key to Escape and Option to Control -- changes that, unlike macOS, iOS can't do but that I need for using Vim
...
- Google Cloud Console to start, stop, and monitor my cloud machine

I used other iOS apps like Spotify, but these were the changes I made for the experiment. After a couple of weeks, I switched out the keyboard for a Logitech that was even lighter.

### Side note: VPNs

VPN support on iOS was good. There was built-in support for L2TP connections and an OpenVPN app that worked well. The connections tended to stay connected. I had no complaints.
...
- I loved that it was *fun*! I can't really explain why, but after using iOS a ton, it came to feel consistent and fun.

- I loved setting up the Workflow app to remote-control my development environment from my phone or iPad. An app release broke this the next day, but it was cool while it lasted.
...
- Touching a screen is cool when I'm doing tablet tasks like reading, but if the iPad is connected to a keyboard, I do *not* want to have to reach up and touch the screen to do everything. Being required to reach up and touch a screen repeatedly for several hours sucks the fun out of iOS. Touch is a pleasing interface while the iPad is in tablet mode, but when connected to a keyboard, touch can't replace a mouse or trackpad.

- There are no truly reliable, full-featured SSH clients, though there are many expensive ones. Blink, the best I could find, constantly broke in various ways and required me to close out my session and establish a new one to fix display problems. For something that itself is a compromise to replace the full-featured native text editors that don't exist on iOS, unreliable SSH apps are a deal-breaker.
...
Vim is great at editing text, and using it instead of an IDE Vim emulator feels like coming home. I can appreciate the Unix philosophy of composing an environment from individual, specialized tools, even if the downside is that nothing feels truly cohesive.

## Can You Code on a ____?

Ultimately, after a few weeks, I stopped using an iPad as my primary device. The limitations were starting to get to me, but what forced me to stop was that my hands ached. It turns out, those tiny Blutooth keyboards hurt to type on after a while!
